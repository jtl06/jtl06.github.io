...
I wrote this article as a short introduction to cache organization and interconnects in modern processors. It assumes elementary knowledge of computer architecture and is part of a series leading up to microbenchmarking core to core latency. Please let me know if you have any feedback. I'm always learning and would love to receive input.
...
![](images/cache-interconnects/interconnects-review.svg){.interconnect-diagram fig-alt="Fully connected, ring, and mesh interconnects, with blue cores and yellow L3 slices."}
...
![](images/cache-interconnects/mesh-distance-review.svg){.mesh-comparison fig-alt="Two identical 4 by 4 meshes showing a nearby and a distant L3 access from the same core, with paths highlighted in purple."}
...
![](images/cache-interconnects/mesh-locality-review.svg){.mesh-comparison fig-alt="Two 6 by 6 meshes comparing global access with access within four 3 by 3 groups. Purple intensity shows the number of requests using each link."}

For this example, the average distance drops from 4.22 to 1.83 mesh hops per request. The average number of requests using each active link drops from 2.62 to 1.69. Darker purple shows more requests using a link. These counts include only the request paths, routed horizontally then vertically, with both directions counted together. This shows why keeping traffic local becomes increasingly important as meshes grow.
...
![](images/cache-interconnects/mesh-concentration-review.svg){.mesh-comparison fig-alt="A 6 by 4 mesh with one core and one L3 slice per junction compared with a 4 by 3 mesh with two cores and two slices per junction, using the same 24 requests."}

Both layouts connect 24 cores and 24 L3 slices. With the same 24 requests and horizontal then vertical routing, average distance drops from 3.62 to 2.54 mesh hops. Average requests per active link rises from 2.35 to 3.59, illustrating the extra traffic concentrated on fewer links. Counts include request paths only, with both directions counted together.
...
![](images/cache-interconnects/cluster-hierarchy-review.svg){.cluster-diagram fig-alt="Four local ring clusters connected through a larger network. A green request stays within one cluster, while a purple request travels between clusters."}

The green request stays within one cluster. The purple request uses the network between clusters to reach its destination.
...
I hope that this article gives you some insight into modern caches and interconnects. There will be a follow up article explaining the behavior of atomic transactions between cores, leading up to an article on microbenchmarking core to core latency.
