...
More broadly, performance behavior on one CPU may not carry over to others, even from the same vendor and using the same microarchitecture. For example, [Intel Xeon 6 with P-cores](https://cdrdv2-public.intel.com/852810/HPC-and-AI-Workloads-with-Intel-Xeon-6-Processors.pdf#page=6) includes designs with one, two, or three compute dies. Requests that cross between dies can incur additional latency. When optimizing code, it is worth checking your assumptions on the target processors.
